...
const getDurations = dir => {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      Object.assign(durations, getDurations(path));
    }
...
const mapEntryToKeyValue = ([fileName, duration]) =>
  `  '${fileName}': ${duration},`;

const keyValues = Object.entries(durationsClean)
  .map(mapEntryToKeyValue)
  .join('\n');

  fs.writeFileSync(
  `${process.cwd()}/src/constants/durations.ts`,
  `export default {
${keyValues}
};
`,
